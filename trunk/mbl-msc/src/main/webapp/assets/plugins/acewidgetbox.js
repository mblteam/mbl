...
ace.widget_boxes = function($) {
	...
	$(document).on('click.ace.widget', '.widget-header a[data-action]', function (ev) {
		...
		if($action == 'collapse') {
			...
			var hide_event_name = 'hide';
			var event
			$box.trigger(event = $.Event(event_name+'.ace.widget'))
			if (event.isDefaultPrevented()) return

			if ($this.parents("div.widget-box").hasClass("open")) {
				
				// $widget_box.toggle(event_name, $this);
				return
			}
			$(".widget-box").removeClass("open");
			$(".widget-toolbar .ace-icon").removeClass("fa-chevron-up").addClass("fa-chevron-down");
			$this.parents("div.widget-box").addClass("open");
			$this.find(".ace-icon").removeClass("fa-chevron-down").addClass("fa-chevron-up");
			
			$(".widget-box").find('.widget-body').each(function(index, el) {
